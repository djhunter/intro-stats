...
>    - No, the p-value is an indication of evidence *against* the null hypothesis.
>- "The p-value is the probability that your result is due to random chance."
>    - No, your result is always the result of random chance (i.e., sampling). The question is whether your result indicates significant evidence against $H_0$, taking random chance into account.

. . .

**Correct Interpretation:** The *p-value* is the probability that you would obtain a sample statistic as or more extreme as the one you observed, assuming that $H_0$ is true.
...
- $H_A$ : Blood thinners have a positive impact on survival: $H_A: \pi_T - \pi_C > 0$

A *two-sided* alternative hypothesis admits the possibility that blood thinners could actually harm the patient.

- $H_A$ : Blood thinners have an impact on survival, either positive or negative, but not zero: $H_A: \pi_T - \pi_C \neq 0$
...
- So it is more conservative to use a two-sided alternative.
...
Consider testing $H_0: \pi = 0.25$ vs. $H_A: \pi > 0.25$. You collect a sample and observe a value of the statistic $\hat{p}$.

1. Open the [Power Simulation Applet](http://www.rossmanchance.com/applets/power.html). Use a sample size of 50, a hypothesized proportion of 0.25, and alternative proportion of 0.33. Choose *proportion of successes* and a *Level of significance* of $\alpha = 0.05$. Draw 5000 samples and click *Count*. 



2. Check *Show alternative*. Assuming that the true proportion is 0.33, what is the probability of failing to reject $H_0$?

3. What is the probability of a Type II error? What is the power of this test? 

4. Name at least one thing the researchers could do to improve the power of this test. Be ready to share.
