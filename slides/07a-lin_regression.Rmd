...
- The residual of a point is the vertical distance between the point and the regression line.
    - Data = Fit + Residual
    - Residual = Data - Fit = Observed - Predicted
...
2. Under *Explore Lines*, check on *show movable line* and show squared residuals.

3. Move the line around and try to make **SSE** as small as possible. (SSE is the sum of the squared residuals, i.e., the total area of all those squares.)
...
- Points at the extremes of the $x$ variable (left or right) are *influential*.
    - Influential points are said to have *leverage*: they tend to tilt the regression line (i.e., change the slope) when they are added/removed.
    - The slope is *not robust* in the presence of influential observations.
    - The slope is more robust to outliers that fall near the middle range of the $x$ variable.
