...
image(qrencode_raster("https://www.rossmanchance.com/applets/2021/oneprop/OneProp.htm"), 
...
1. Open the [One Proportion Applet](https://www.rossmanchance.com/applets/2021/oneprop/OneProp.htm), set the *probability of heads/success* to 0.6, the *Sample size* to 100, and leave *Number of samples* at 1. *Uncheck Show animation*.

2. Before clicking *Draw Samples*, predict the most likely number of successes and failures in 100 tries, if the probability of success is 0.6.

3. Click the radio button for *Proportion of successes* and click *Draw Samples*. How close was the simulated sample from your prediction in 2?

4. Now set *Number of samples* to 10000 and click *Draw Samples*. Check the *Summary Statistics* box and record the Mean and Standard Deviation (SD). 

5. Does the Central Limit Theorem apply? (i.e., are there enough expected successes and failures?) Check the *Normal Approximation* box and observe how well it fits.

6. Now *Reset* the applet and repeat 1-5 for a success probability of 0.2 and a sample size of 10? How is your answer to 5 different?
...
1. Compute a quick 95% confidence interval for the true percentage of U.S. adults who suffer from chronic illness. Write the confidence interval in the form $(L, R)$.

2. Write the confidence interval in the form: $\text{estimate} \pm \text{margin of error}$
