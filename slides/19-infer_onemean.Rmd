...
- The 95% *SE confidence interval* is the quick approximation $\mbox{point estimate} \pm 2 \cdot SE$
...
1. Open the [Bootstrapping Applet](https://www.rossmanchance.com/applets/2021/sampling/OneSample.html?population=bootstrap). *Clear* the preloaded data and paste in the above price data (including the header). *Use Data* and record $\bar{x}$.

2. Check *Show Sampling Options* and make a bootstrap distribution of sample means from this sample. Use at least 1000 $\bar{x}_\text{boot}$'s.

3. Use the SD from the distribution to estimate the standard error of $\bar{x}$, and calculate a 95% SE confidence interval for the mean price.
