...
- We can estimate the variability of the statistics $b_0$ and $b_1$ using randomization.
    ...
    - Bootstrapping, or
    - Randomizing the data to create a null distribution
...
**Research Question:** Can we predict the weight of a newborn baby based on the number of weeks of gestation?

1. What are the explanatory and response variables, and what are their types?

2. The observed statistics are the regression slope (0.34) and the regression intercept (-5.72). What symbols do we use for these numbers?

3. Write down the regression equation.

4. Predict the weight of a baby born after 30 weeks of gestation.

5. Interpret the regression slope in context.

## Hypotheses

>- The regression slope and intercept are based on a *random sample*.
>- For our random sample, the regression slope was positive ($b_1 \approx 0.34$), but a different random sample would give us a different slope.
>- Do we have enough evidence to conclude that the actual (population) slope $\beta_1$ is not zero?
>- Is there an association between weeks of gestation and the weight of the baby?
>- Null Hypothesis: No linear association
>- $H_0: \beta_1 = 0$ (Population regression slope is zero)
